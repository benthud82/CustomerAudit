...
$dmgaccresult = $conn1->prepare("SELECT 
                                        A.RETURNCODE,
                                        B.TRUNCATEDDESC,
                                        sum(case
...
$dmgaccresult->execute();
$dmgaccresultarray = $dmgaccresult->fetchAll(pdo::FETCH_ASSOC);

$minpercent = .9985;  //if percentage below, then issue
$issuecount_dmgacc = 0; //initialize count of issues.
foreach ($dmgaccresultarray as $key => $value) {
    $var_returncode = $dmgaccresultarray[$key]['RETURNCODE'];
    $var_return_mnth = $dmgaccresultarray[$key]['RETURNS_MNTH'];
    $var_return_qtr = $dmgaccresultarray[$key]['RETURNS_QTR'];
    $var_return_year = $dmgaccresultarray[$key]['RETURNS_YEAR'];
    $var_description = $dmgaccresultarray[$key]['TRUNCATEDDESC'];
...
if ($issuecount_dmgacc == 0) { //add classes to dmgacc card if no issues present    
    echo "<li style='text-align: left;'>There are no damage accuracy opportunities</li>
       <script>
        $('#panel_dmgacc').addClass('bg-success'); 
        $('#icon_dmgacc').addClass('fa-check fa-2x'); 
    </script>";
} else { //add classes to dmgacc card if issues are present
    echo "<script>
        $('#panel_dmgacc').addClass('bg-danger');
        $('#icon_dmgacc').addClass('fa-times fa-2x');
    </script>";
}

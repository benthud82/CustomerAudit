...
//if score has decreased, find min value to determine driver of lower score
        ...
        $driverlookuparray[$driverkey]['STMT'] = '';
    }
}
...
foreach ($top3_array as $key => $value) {
    ...
    $fr_incdec = ($fr_end >= $fr_start ? ' increased ' : ' decreased ');
    $fr_incdec2 = ($fr_end >= $fr_start ? ' increase ' : ' decrease ');
    ?>

    <p> 
        <?php echo $salesplan_desc . ' (' . $salesplan . ')' ?> has seen a <strong><?php echo $incdec ?></strong> in their customer score to <strong><?php echo $currscore ?></strong> this quarter from <strong><?php echo $histscore ?></strong> last quarter.
        The fill rate has <strong><?php echo $fr_incdec ?></strong> from <strong><?php echo $fr_end ?>%</strong> to <strong><?php echo $fr_start ?>%</strong>. <?php echo $driver_stmt ?>
    </p>
    <?php
}
?>
